...
TASK = {
   ...
   'grapheme': Struct(
       num_class = 1295,
       class_map = dict(pd.read_csv('../input/bengaliaitop/grapheme_1295.csv')[['grapheme','label']].values),
       #class_map = dict(pd.read_csv(DATA_DIR + 'class_map.csv')[['component','label']].values),
       #freqency  = None,
   ),
}
NUM_TASK  = len(TASK)
...
class KaggleDataset(Dataset):
    def __init__(self, split, mode, csv, parquet, augment=None):
        ...
        if split is not None:
            s  = np.load('data/split/%s'%split, allow_pickle=True)
...
class BalanceSampler(Sampler):
    def __init__(self, dataset, length):
        ...
        #print(df.shape)
        #print(len(df['grapheme'].unique()))
        #pd.DataFrame({'grapheme' :df['grapheme'].unique()}).to_csv('../grapheme_1295.csv')
        #print(df.head(5))
